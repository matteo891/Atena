...
> **Ultimo aggiornamento:** 2026-04-30 — commit `1615206` (test(golden): mini-Samsung snapshot pipeline + fix allocator qty=0). Tag: `milestone/stack-frozen-v0.9.0` + `milestone/first-formula-v1.0.0` + **`milestone/pipeline-e2e-v1.1.0`** + **6 checkpoint**. Catena CHG odierna: 001→...→**041**. **Tabelle Allegato A coperte: 10/10** ✓ + **388 test PASS** (354 unit/governance + 21 integration + **13 golden**). **Indice GitNexus** (1907 / 2203 — stale ~24 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **🛡️ PIPELINE BLINDATA**: `tests/golden/test_pipeline_samsung_mini.py` snapshot byte-exact 10 ASIN (R-04/R-05/R-08/F5 floor/saturazione/panchina coperti) — sentinella forte contro drift su `run_session`. Fix bug regression in `allocate_tetris` Pass 2 (qty_final=0 ora skippato, S010_TINY scenario). Test golden marker `pytest -m golden`. Differenziato da `test_value_chain` (scalare) — sentinelle complementari.
...
| **NEXT** | **Prossimi step possibili** | Pipeline blindata + UI MVP | (m) **persistenza `SessionResult`** in DB (consumer modelli `sessions`/`vgp_results`/`cart_items`/`panchina_items`/`storico_ordini` Allegato A); (d) telemetria `session_started`/`tetris_break_saturation`/`veto_roi_applied` con structlog dispatch (ADR-0021); (q) refactor UI multi-page ADR-0016 (`pages/`, `components/`, `state.py` + caching `@st.cache_data`); (r) acquisizione dati reale: `io_/extract` Samsung (Playwright + Tesseract + Keepa, ADR-0017) — last big block; (s) golden Samsung 1000 ASIN (post io_/extract); (c) lookup `config_overrides` runtime per soglia ROI; (e) lookup `Referral_Fee` per categoria; chiusi: (a/a'/a''/b/b''/f/g/h/i/j/k/l/n/p) |
