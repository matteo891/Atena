...
> **Ultimo aggiornamento:** 2026-04-30 — commit `cdeae5e` (feat(tetris,governance): telemetria tetris.skipped_budget + fix regex MULTILINE). Tag: 3 milestone + 7 checkpoint. Catena CHG odierna: 001→...→**046**. **Tabelle Allegato A coperte: 10/10** ✓ + **420 test PASS** (372 unit/gov/golden + 48 integration). **Indice GitNexus** (1907 / 2203 — stale ~36 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **🛡️ TELEMETRIA + GOVERNANCE HARDENED**: primo evento canonico vivente del catalogo ADR-0021 (`tetris.skipped_budget` emesso da `allocate_tetris` Pass 2 over-budget con `asin`/`cost`/`budget_remaining`). Bug regex MULTILINE in `test_log_events_catalog` fixato (era no-op silenzioso per `continue`). Orchestrator `continue` defensive sostituito con `raise RuntimeError` (R-01 strict). 3 test caplog.
...
| **NEXT** | **Prossimi step possibili** | Telemetria pattern aperto | (d') **telemetria aggregata in `compute_vgp_score`** (`vgp.veto_roi_failed` count, `vgp.kill_switch_zero` count) + `panchina.archived` per panchina (richiede emissione summary, non per-riga); (z) **migrazione a `structlog.bind(session_id, tenant_id, asin)`** per context tracing (sostituisce stdlib logging); (v) `UNIQUE(listino_hash)` migration + upsert idempotente; (q) refactor UI multi-page ADR-0016 (`pages/`, `components/`, `state.py` + caching `@st.cache_data`); (r) `io_/extract` Samsung (Playwright + Tesseract + Keepa) — last big block; (s) golden Samsung 1000 ASIN; (c) lookup `config_overrides` runtime per soglia ROI; (e) lookup `Referral_Fee` per categoria; (y) `load_session_full`; chiusi: (a/a'/a''/b/b''/d/f/g/h/i/j/k/l/m/n/p/t/u/x) |
