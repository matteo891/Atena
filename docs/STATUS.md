...
> **Ultimo aggiornamento:** 2026-04-30 — commit `4747382` (feat(tetris): inaugura cluster con allocator R-04 + R-06 greedy). Tag: `milestone/stack-frozen-v0.9.0` + **`milestone/first-formula-v1.0.0`** + **6 checkpoint** (01..06; 06 su `0812f5d`). Catena CHG odierna: 001→...→**036**. **Tabelle Allegato A coperte: 10/10** ✓ + **307 test PASS** (286 unit/governance + 21 integration). **Indice GitNexus** (1907 / 2203 / 13 / 4 — behind di ~7 commit dopo CHG-034/035/036 + governance).
> **Sessione corrente:** TALOS — **cluster `tetris/` inaugurato**: `allocate_tetris(vgp_df, budget, locked_in)` greedy con Pass 1 R-04 (locked-in priorità∞) + Pass 2 R-06 (VGP DESC, saturazione 99.9%, continue letterale su over-budget). `Cart` (mutable) + `CartItem` (frozen) + `InsufficientBudgetError` fail-fast (R-01). 19 test. Cluster `vgp/` completo (CHG-034 normalize, CHG-035 score). Pronto per `tetris/panchina.py` (R-09) o orchestratore di sessione end-to-end.
...
| **NEXT** | **Prossimi step possibili** | Catena Leader-driven in corso | (b'') `tetris/panchina.py` R-09 archivio idonei scartati per capienza (consumatore diretto di `Cart` + `vgp_df`); (c) lookup `config_overrides` per soglia ROI runtime override (sbloccato da CHG-031); (d) telemetria evento `veto_roi_applied` + `tetris_break_saturation` con primo orchestrator pipeline; (e) lookup `Referral_Fee` per categoria; (j) F4 Quantità Target + F4.A/B/C; (k) F5; (l) orchestratore di sessione end-to-end (compute_vgp → allocate_tetris → persiste Cart + Budget_T+1); chiusi: (a/a'/a''/b/f/g/h/i) |
