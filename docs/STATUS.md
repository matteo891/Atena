...
> **Ultimo aggiornamento:** 2026-05-01 — post-CHG-2026-05-01-006 (Fase 1 Path B aperta: fallback chain orchestratrice `lookup_product`). Tag: 7 milestone + **11 checkpoint** (invariato post-checkpoint-11). Catena CHG 2026-04-30: 001→059 (chiusa). Catena CHG 2026-05-01: 001..005 (chiusa primitive + telemetria) + **006** (fallback chain mock-only, no live adapters). **Tabelle Allegato A: 10/10** ✓ + **639 test PASS** (534 unit/gov/golden + 105 integration; era 624, +15). **Catalogo eventi canonici ADR-0021: 10/11 viventi** (invariato). **Indice GitNexus reindexato** a inizio sessione (3755 nodes / 4769 edges / 67 clusters / 8 flows).
> **Sessione corrente:** TALOS round 2 (modalità "macina" riautorizzata Leader 2026-05-01) — Leader ha ratificato **Path B** ("obiettivo prodotto funzionante") come MVP target. Sequenza in 3 fasi: Fase 1 (mock-testabile, no setup), Fase 2 (installazioni di sistema in sospeso), Fase 3 (live adapters + 5 decisioni Leader pre-flight). **CHG-2026-05-01-006** = primo CHG di Fase 1: `lookup_product(asin, *, keepa, scraper, page, ocr) -> ProductData` orchestra Keepa primario + AmazonScraper fallback su `buybox`/`title` + OCR placeholder. R-01 fail-fast su rate-limit/transient, miss accumulati in `notes`. Zero nuove deps, zero nuovi eventi canonici.
...
- **CHG-2026-05-01-006** `TBD`: `src/talos/io_/fallback_chain.py` con `lookup_product(asin, *, keepa, scraper, page, ocr) -> ProductData`. Orchestra Keepa primario + Scraper fallback su buybox/title (OCR placeholder, non invocato in CHG-006). 15 test unit mock-only. **639 PASS**.
