...
> **Ultimo aggiornamento:** 2026-04-30 — commit `da0a370` (feat(ui): inaugura cluster con dashboard Streamlit mono-page MVP). Tag: `milestone/stack-frozen-v0.9.0` + `milestone/first-formula-v1.0.0` + **`milestone/pipeline-e2e-v1.1.0`** (su `6584d49`) + **6 checkpoint**. Catena CHG odierna: 001→...→**040**. **Tabelle Allegato A coperte: 10/10** ✓ + **374 test PASS** (353 unit/governance + 21 integration). **Indice GitNexus** (1907 / 2203 — stale ~21 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **🎯 STRATO VISIVO INAUGURATO**: `src/talos/ui/dashboard.py` Streamlit mono-page MVP. Sidebar parametri sessione (budget/velocity/threshold/lot) + file upload CSV + chiamata `run_session` + metric saturazione/budget_t1 + tabelle Cart/Panchina/enriched. streamlit>=1.40 introdotto. Lancio: `uv run streamlit run src/talos/ui/dashboard.py`. Refactor multi-page ADR-0016 + caching + RLS bootstrap = scope CHG successivi.
...
| **NEXT** | **Prossimi step possibili** | UI MVP funzionale | (m) **persistenza `SessionResult`** in DB (consumer modelli `sessions`/`vgp_results`/`cart_items`/`panchina_items`/`storico_ordini` Allegato A); (p) **golden test Samsung byte-exact** (ADR-0019: dataset sintetico 1k ASIN + snapshot Cart/Panchina/Budget_T+1); (d) telemetria `session_started`/`tetris_break_saturation`/`veto_roi_applied` con structlog dispatch (ADR-0021); (q) refactor UI multi-page ADR-0016 (`pages/`, `components/`, `state.py` + caching `@st.cache_data`); (r) acquisizione dati reale: `io_/extract` Samsung (Playwright + Tesseract + Keepa, ADR-0017) — last big block; (c) lookup `config_overrides` runtime per soglia ROI; (e) lookup `Referral_Fee` per categoria; chiusi: (a/a'/a''/b/b''/f/g/h/i/j/k/l/n) |
