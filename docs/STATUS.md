...
> **Ultimo aggiornamento:** 2026-04-30 — commit `9a55139` (feat(persistence,ui): load_session_by_id + UI dettaglio). Tag: 3 milestone + 7 checkpoint. Catena CHG odierna: 001→...→**045**. **Tabelle Allegato A coperte: 10/10** ✓ + **417 test PASS** (369 unit/gov/golden + 48 integration). **Indice GitNexus** (1907 / 2203 — stale ~34 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **🎯 CRUD-LIGHT PERSISTENZA COMPLETO**: `LoadedSession` + `load_session_by_id(db_session, sid, *, tenant_id=1) -> LoadedSession | None`. UI dashboard con expander "Storico Sessioni" + `number_input` ID + bottone "Carica dettaglio" → `_render_loaded_session_detail` (metric + tabelle Cart/Panchina). Cluster `persistence/` ora implementa create+list+load. Storico append-only (no update/delete by design).
...
| **NEXT** | **Prossimi step possibili** | CRUD-light persistenza completo | (d) **telemetria** `session_started`/`tetris_break_saturation`/`veto_roi_applied` con structlog dispatch (ADR-0021); (v) `UNIQUE(listino_hash)` migration + upsert idempotente; (q) refactor UI multi-page ADR-0016 (`pages/`, `components/`, `state.py` + caching `@st.cache_data`); (r) `io_/extract` Samsung (Playwright + Tesseract + Keepa) — last big block; (s) golden Samsung 1000 ASIN; (c) lookup `config_overrides` runtime per soglia ROI; (e) lookup `Referral_Fee` per categoria; (y) `load_session_full` (ricostruzione `SessionResult` da DB per re-execute pipeline); chiusi: (a/a'/a''/b/b''/f/g/h/i/j/k/l/m/n/p/t/u/x) |
