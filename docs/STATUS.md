...
> **Ultimo aggiornamento:** 2026-05-01 round 6 — **CHG-033 (B1.1.d): bridge stdlib→structlog su `extract/` + rinomina test**. HEAD `0d748f2`. Migrati `extract/{samsung,asin_resolver}.py` (1 emit canonico `extract.kill_switch` R-05 + 1 init-only anti-dead-code). Rinominato `test_io_extract_telemetry.py` → `test_extract_telemetry.py` via `git mv` + riscrittura caplog → LogCapture (4 test invariati semanticamente). **Pattern uniforme post-bridge raggiunto sui 4 cluster `vgp`/`tetris`/`io_`/`extract`** — manca solo `ui/` (B1.1.e). **838 test PASS invariato** (700 + 138). Comportamento applicativo invariato 100%. Detect_changes risk LOW (0 processi affetti). Catalogo ADR-0021 invariato (17/17). 5 CHG significativi dal checkpoint-16 (CHG-029, 030, 031, 032, 033 — soglia raggiunta, **checkpoint-17 atteso post-CHG-033**). Tag: **8 milestone** + **16 checkpoint** (`checkpoint/2026-05-01-16` su `ceb8c7c`).
...
| **CHECKPOINT-17** | **Soglia 5 CHG significativi raggiunta dal checkpoint-16** | da creare | Finestra: CHG-029..033 (count family + 4 CHG B1.1.a-d). Tag `checkpoint/2026-05-01-17` da creare e pushare. |
