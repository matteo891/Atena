...
> **Ultimo aggiornamento:** 2026-04-30 — commit `5829bfe` (feat(vgp): formula composita `compute_vgp_score` con R-05 + R-08 vettoriale). Tag: `milestone/stack-frozen-v0.9.0` + **`milestone/first-formula-v1.0.0`** + **5 checkpoint** (01..05 target `cf11e6c`). Catena CHG odierna: 001→...→**035**. **Tabelle Allegato A coperte: 10/10** ✓ + **288 test PASS** (267 unit/governance + 21 integration). **Indice GitNexus operativo** (1907 / 2203 / 13 / 4 — reindex post `aeb2dbb`, ora behind di 4 commit dopo CHG-034/035 + governance).
> **Sessione corrente:** TALOS — **cluster vettoriale `vgp/` esteso a scoring composito**: numpy 2.4 + pandas 2.3 (CHG-034), `min_max_normalize` L04b (CHG-034), **`compute_vgp_score`** verbatim PROJECT-RAW sez. 6.3 con pesi 0.4/0.4/0.2 + R-05 KILL-SWITCH + R-08 VETO ROI vettoriale (CHG-035). Primo consumatore architetturale di `min_max_normalize` su 3 colonne. 17 test (15 snapshot + 2 property-based). Pronto per `tetris/allocator.py` (consumatore di `vgp_score` ordinato).
...
| **NEXT** | **Prossimi step possibili** | Catena Leader-driven in corso | (a'') `tetris/allocator.py` con R-06 saturazione 99.9% + R-04 locked-in priorità∞ — consumatore diretto di `vgp_score` ordinato; (b'') `tetris/panchina.py` R-09 archivio idonei scartati; (c) lookup `config_overrides` per soglia ROI runtime override (sbloccato da CHG-031); (d) telemetria evento `veto_roi_applied` con primo orchestrator pipeline; (e) lookup `Referral_Fee` per categoria; (j) F4 Quantità Target + F4.A/B/C; (k) F5; (l) orchestratore di sessione + persistenza Budget_T+1; chiusi: (a/a'/b/f/g/h/i) |
