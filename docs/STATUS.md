...
> **Ultimo aggiornamento:** 2026-04-30 — commit `00a3c3f` (feat(tetris): completa cluster con build_panchina R-09). Tag: `milestone/stack-frozen-v0.9.0` + **`milestone/first-formula-v1.0.0`** + **6 checkpoint** (01..06; 06 su `0812f5d`). Catena CHG odierna: 001→...→**037**. **Tabelle Allegato A coperte: 10/10** ✓ + **317 test PASS** (296 unit/governance + 21 integration). **Indice GitNexus** (1907 / 2203 / 13 / 4 — stale ~10 commit dopo CHG-034..037 + governance; reindex utile).
> **Sessione corrente:** TALOS — **cluster `tetris/` completo**: `allocate_tetris` (CHG-036) + `build_panchina` (CHG-037 R-09 verbatim, ASIN idonei vgp_score>0 non in cart, ordinati DESC). Pipeline core di sessione assemblabile: `min_max_normalize` → `compute_vgp_score` → `allocate_tetris` → `build_panchina`. Manca solo orchestratore che precalcoli F1/F2/F4/F5 + `velocity_monthly` per chiudere il loop end-to-end.
...
| **NEXT** | **Prossimi step possibili** | Catena Leader-driven in corso | (j) F4 Quantità Target + F4.A (`Q_m = v_tot/(s_comp+1)`) + velocity_monthly + F5 (`(qty_target // 5) * 5`) — prerequisito orchestratore; (l) orchestratore di sessione end-to-end (`session.py` o `orchestrator.py`: precalcolo F1/F2/F4/F5 + compute_vgp_score + allocate_tetris + build_panchina + compounding_t1 + persistenza); (c) lookup `config_overrides` per soglia ROI runtime override (sbloccato da CHG-031); (d) telemetria evento `veto_roi_applied` + `tetris_break_saturation` (richiede orchestratore); (e) lookup `Referral_Fee` per categoria; (k) F5; chiusi: (a/a'/a''/b/b''/f/g/h/i) |
