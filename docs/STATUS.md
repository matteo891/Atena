...
> **Ultimo aggiornamento:** 2026-05-01 — **Fase 1 Path B CHIUSA** post-CHG-2026-05-01-010 (`acquire_and_persist` orchestratore di acquisizione+persistenza) + tag `checkpoint/2026-05-01-12` su `e7d42e4`. Tag: 7 milestone + **12 checkpoint** (era 11). Catena CHG 2026-04-30: 001→059 (chiusa). Catena CHG 2026-05-01: 001..005 (primitive+telemetria) + **006..010 (Fase 1 Path B chiusa: fallback chain + asin kwarg + bridge + bulk + orchestratore)**. **Tabelle Allegato A: 10/10** ✓ + **660 test PASS** (548 unit/gov/golden + 112 integration; era 624, +36 totale). **Catalogo eventi canonici ADR-0021: 10/11 viventi** (invariato). **Indice GitNexus reindexato** a inizio sessione (3755 nodes / 4769 edges / 67 clusters / 8 flows).
...
| TEST-DEBT-001 | `sudo playwright install-deps chromium` (system libs libnspr4/libnss3/etc.) | **Posticipato Leader 2026-05-01.** Sblocca 6 test live `test_live_playwright.py` attualmente SKIPPED. ~5 min sudo. |
| TEST-DEBT-002 | Test live `AmazonScraper.scrape_product` su HTML Amazon.it reale (golden statici) | **Posticipato 2026-05-01.** I selettori `selectors.yaml` CHG-002/013 sono best-guess sull'analisi pubblica HTML Amazon; mai validati contro pagina reale. Rischio drift selettori non rilevato. ~½-1 sessione + golden fixtures. |
| TEST-DEBT-003 | Smoke test UI Streamlit live in browser (Path A end-to-end CSV → cruscotto) | **Posticipato 2026-05-01.** UI mai aperta in browser durante questa sessione. Streamlit ha caching/state che può nascondere bug. ~30 min + fix piccoli. |
| TEST-DEBT-004 | Test live `_LiveKeepaAdapter` + mapping CSV indici Keepa | **Bloccato in attesa sandbox API key Keepa (Leader, in arrivo).** CHG-014 atteso. |
