...
> **Ultimo aggiornamento:** 2026-05-01 round 4 — **Keepa private API key consegnata + .env loading attivato**. HEAD `4947f27` (chore refresh GitNexus stats; CHG-014 in commit successivo). Tag: 7 milestone + 13 checkpoint (`checkpoint/2026-05-01-13` su `4691bd4`). **690 test PASS, 0 SKIPPED** (568 unit/gov/golden + 122 integration; +3 test settings .env loading). Catena CHG 2026-05-01: 001..014. **Catalogo eventi canonici ADR-0021: 10/11 viventi** (invariato). **Indice GitNexus fresh** (4143 nodes / 5393 edges / 73 clusters / 16 flows). **Path B scraping-only ~92-94% utilizzabile validato live** + Keepa channel sblocco imminente (CHG-015 `_LiveKeepaAdapter` next).
...
| TEST-DEBT-004 | Test live `_LiveKeepaAdapter` + mapping CSV indici Keepa | **Sbloccato 2026-05-01 round 4: Keepa private API key consegnata** + CHG-014 caricamento .env attivato. CHG-015 `_LiveKeepaAdapter` next. Decisione Leader pendente: idx 18 (BUY_BOX_SHIPPING) vs idx 0 (AMAZON) per `buy_box`; idx 3 (SALES) per `bsr` root; parsing `fee_fba` da `data` field. |
