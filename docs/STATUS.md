...
> **Ultimo aggiornamento:** 2026-04-30 — commit `ac3a0ef` (feat(vgp,tetris): telemetria 3 eventi canonici). Tag: 3 milestone + 8 checkpoint. Catena CHG odierna: 001→...→**049**. **Tabelle Allegato A coperte: 10/10** ✓ + **438 test PASS** (380 unit/gov/golden + 58 integration). **Indice GitNexus** (1907 / 2203 — stale ~42 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **📡 CATALOGO EVENTI 4/10 VIVENTI**: `vgp.veto_roi_failed` (asin/roi_pct/threshold) emesso da `compute_vgp_score` per riga vetata. `vgp.kill_switch_zero` (asin/match_status) per riga killed (R-05). `panchina.archived` (asin/vgp_score) per riga in panchina post-sort. +`tetris.skipped_budget` (CHG-046). Level DEBUG (silente in produzione, capturable test). 5 dormienti (extract/keepa/scrape/ocr/db.audit) attendono moduli futuri.
...
| **NEXT** | **Prossimi step possibili** | Catalogo eventi 4/10 viventi | (β) **`upsert_session`** decisione Leader semantica; (z) migrazione a `structlog.bind(session_id, tenant_id)` context tracing (sostituisce stdlib logging); (q) refactor UI multi-page ADR-0016; (r) **`io_/extract` Samsung** (Playwright + Tesseract + Keepa) — last big block, attiva eventi `extract.*`/`keepa.*`/`scrape.*`/`ocr.*`; (s) golden Samsung 1000 ASIN; (c) lookup `config_overrides` runtime per soglia ROI; (e) lookup `Referral_Fee` per categoria; (y) `load_session_full`; chiusi: (a/a'/a''/b/b''/d/d'/f/g/h/i/j/k/l/m/n/p/t/u/v/x/α) |
