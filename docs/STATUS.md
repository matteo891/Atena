...
> **Ultimo aggiornamento:** 2026-05-01 — commit `[pending]` (feat: SamsungExtractor + R-05 CHG-2026-05-01-004). Tag: 4 milestone + 10 checkpoint. Catena CHG 2026-04-30: 001→059 (chiusa). Catena CHG 2026-05-01: 001..**004** (in corso). **Tabelle Allegato A coperte: 10/10** ✓ + **609 test PASS** (509 unit/gov/golden + 100 integration). **Indice GitNexus** stale ~7 commit.
> **Sessione corrente:** TALOS oltre-mezzanotte (modalità "macina" clausola sessione, prosegue da 2026-04-30 sera) — primo CHG del blocco strategico `io_/extract` Samsung (ADR-0017). **CHG-2026-05-01-001 = `KeepaClient` skeleton**: adapter pattern + rate limit hard (`pyrate-limiter`) + retry esponenziale (`tenacity`) + 3 errori R-01 espliciti. Decisioni Leader D1-D5 ratificate "default" (memory `project_io_extract_design_decisions.md`).
...
| **🧬 `SamsungExtractor` + R-05 KILL-SWITCH (ADR-0017 + ADR-0018). Inaugura `src/talos/extract/`. NLP regex + `rapidfuzz` (D4.a) per estrazione modello/RAM/ROM/colore/connettivita'/enterprise; whitelist YAML versionata `samsung_whitelist.yaml` (D4.b: 20 modelli 5G + 17 colori + RAM/ROM canonici); `match()` con weighted sum (D4.c: model=3, ram=2, rom=2, color=1, conn=1) e soglie SICURO/AMBIGUO/MISMATCH; **R-05 hard su model mismatch** -> caller forzera' VGP=0 + evento `extract.kill_switch` (CHG-005). Dep `rapidfuzz>=3,<4`. 31 test unit (whitelist + parse + match + R-05 + end-to-end). 609 PASS (509 + 100).** | 0017, 0018, 0014, 0019, 0021 | [CHG-2026-05-01-004](changes/2026-05-01-004-samsung-extractor.md) | `[pending]` |
