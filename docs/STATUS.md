...
> **Ultimo aggiornamento:** 2026-04-30 — commit `1bdac33` (feat(persistence,ui): config_overrides runtime + UI persistente soglia ROI). Tag: **4 milestone** (+ `crud-and-telemetry-v1.2.0` su `6654795`) + 8 checkpoint. Catena CHG odierna: 001→...→**050**. **Tabelle Allegato A coperte: 10/10** ✓ + **445 test PASS** (380 unit/gov/golden + 65 integration). **Indice GitNexus** (1907 / 2203 — stale ~46 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **🎛️ CONFIGURABILITÀ PERSISTENTE APERTA**: `config_repository.py` con `get_config_override_numeric` + `set_config_override_numeric` (UPSERT). UI sidebar pre-carica soglia veto ROI da DB tenant + bottone "Salva soglia ROI come default tenant". L10 PROJECT-RAW chiusa operativamente. Bug fix migration: `idx_config_unique` ricreato con `NULLS NOT DISTINCT` (Postgres 15+) per UPSERT funzionante con `scope_key=NULL`.
