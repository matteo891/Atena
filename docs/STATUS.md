...
> **Ultimo aggiornamento:** 2026-04-30 — commit `6584d49` (feat(orchestrator): pipeline end-to-end run_session, gap ADR risolto). Tag: `milestone/stack-frozen-v0.9.0` + **`milestone/first-formula-v1.0.0`** + **6 checkpoint** (01..06; 06 su `0812f5d`). Catena CHG odierna: 001→...→**039**. **Tabelle Allegato A coperte: 10/10** ✓ + **366 test PASS** (345 unit/governance + 21 integration). **Indice GitNexus** (1907 / 2203 — stale ~16 commit dopo CHG-034..039 + governance; reindex consigliato).
> **Sessione corrente:** TALOS — **🎯 PIPELINE END-TO-END FUNZIONALE**: `src/talos/orchestrator.py` con `run_session(SessionInput) -> SessionResult` compone tutti i cluster (formulas + vgp + tetris + compounding) in un singolo entrypoint. Gap ADR su orchestratore risolto inline 2026-04-30 (Leader ratifica opzione A: file top-level, no directory). Primo punto di ingresso utilizzabile dalla UI Streamlit + persistenza DB + telemetria. 20 test end-to-end con scenario Samsung-like.
...
| **NEXT** | **Prossimi step possibili** | Pipeline core funzionale | (m) **persistenza `SessionResult`** in DB (orchestratore consumer dei modelli `sessions`/`vgp_results`/`cart_items`/`panchina_items`/`storico_ordini` Allegato A); (n) **UI Streamlit minima** (`src/talos/ui/dashboard.py` ADR-0016: input listino raw via file upload + slider velocity/budget + tabella Cart + tabella Panchina + cell budget_t1); (d) telemetria evento `session_started`/`tetris_break_saturation`/`veto_roi_applied` con structlog dispatch (ADR-0021); (c) lookup `config_overrides` per soglia ROI runtime override (sbloccato da CHG-031); (e) lookup `Referral_Fee` per categoria; (o) golden test Samsung byte-exact (ADR-0019); chiusi: (a/a'/a''/b/b''/f/g/h/i/j/k/l) |
