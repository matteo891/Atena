...
> **Ultimo aggiornamento:** 2026-05-01 round 5+ — **CHG-024 telemetria additiva UI** (errata ADR-0021 catalogo 13→15 voci) + manutenzione (memory MVP refresh + GitNexus reindex). HEAD `065711b`. **801 test PASS** (663 unit/gov/golden + 138 integration con live attivi end-to-end; +4 test rispetto a CHG-023). **Catalogo eventi canonici ADR-0021: 15/15 voci** (aggiunti `ui.override_applied` + `ui.resolve_failed`). **Indice GitNexus fresh** (4770 nodes / 6213 edges / 88 clusters / 16 flows). Modalità "macina" round 5+ è clausola di sessione, **NON persiste**. Tag: **8 milestone** + **15 checkpoint** (`checkpoint/2026-05-01-15` su `4c822f4`); 1 CHG significativo dal checkpoint-15 (soglia: 5).
