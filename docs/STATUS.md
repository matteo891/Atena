...
> **Ultimo aggiornamento:** 2026-04-30 sera — commit `9a3b0c3` (feat: delete_config_override + UI Reset CHG-054). Tag: 4 milestone + 9 checkpoint. Catena CHG odierna: 001→...→**054**. **Tabelle Allegato A coperte: 10/10** ✓ + **475 test PASS** (387 unit/gov/golden + 88 integration). **Indice GitNexus** stale ~5 commit dopo i CHG odierni (reindex 2026-04-30 sera).
> **Sessione corrente:** TALOS sera (modalità "macina" autorizzata) — Round CHG-052..054. **CRUD-light persistence completo (READ + WRITE + DELETE)**, **L12 chiusa lato pipeline + UI Reset**, override DB-driven gestibili end-to-end dal cruscotto CFO. Manca solo aggancio orchestrator+UI per `referral_fee_overrides` automatico (post `io_/extract`).
