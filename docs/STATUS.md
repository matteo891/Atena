...
| **NEXT** | **Prossimi step possibili** | In attesa | (a) `vgp/normalize.py` versione vettoriale L04b min-max (richiede Numpy/pandas — primo dep applicativo); (b) ~~refactor `engine.py` via `TalosSettings`~~ chiuso CHG-030; (c) lookup `config_overrides` per soglia ROI runtime override (sbloccato da CHG-031); (d) telemetria evento `veto_roi_applied` con primo orchestrator pipeline; (e) lookup `Referral_Fee` per categoria; (f) F3 Compounding T+1; (g) ~~refactor `scripts/db_bootstrap.py` via settings~~ chiuso CHG-031; (h) `milestone/first-formula-v1.0.0` |
