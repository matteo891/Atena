...
> **Ultimo aggiornamento:** 2026-04-30 — commit `<pending CHG-012>` (config_overrides + RLS + UNIQUE INDEX composito). Tag: `milestone/stack-frozen-v0.9.0` + `checkpoint/2026-04-30-01`. Catena CHG odierna: 001→002→003→004→005→006→007→008→009→010→011→012. Tabelle Allegato A coperte: 4/10 (`sessions`, `asin_master`, `listino_items`, `config_overrides`)
...
| **Quarta tabella: `ConfigOverride` (primo con RLS Zero-Trust + indice UNIQUE composito 4 col + revision `027a145f76a8`). 15 test unit.** | 0015, 0014, 0013, 0019 | [CHG-2026-04-30-012](changes/2026-04-30-012-config-overrides-model-with-rls.md) | `<pending>` |
