...
> **Ultimo aggiornamento:** 2026-04-30 — commit `7bd60dc` (feat(vgp): inaugura cluster vettoriale — `min_max_normalize` L04b). Tag: `milestone/stack-frozen-v0.9.0` + **`milestone/first-formula-v1.0.0`** + **5 checkpoint** (01..05 target `cf11e6c`). Catena CHG odierna: 001→...→**034**. **Tabelle Allegato A coperte: 10/10** ✓ + **271 test PASS** (250 unit/governance + 21 integration). **Indice GitNexus operativo** (1907 / 2203 / 13 / 4 — reindex post `aeb2dbb`).
> **Sessione corrente:** TALOS — **frontiera vettoriale aperta**: numpy 2.4 + pandas 2.3 introdotte come prime deps applicative pesanti; `vgp/normalize.py` con `min_max_normalize` verbatim ADR-0018 (L04b min-max [0,1] esclude righe KILLED da min/max, edge case `eligible vuoto`/`max==min` → 0.0); 13 test (10 snapshot + 3 property-based Hypothesis). Primo modulo Talos su `pd.Series`. CHG-034.
...
| **NEXT** | **Prossimi step possibili** | Catena Leader-driven in corso | (a') `vgp/score.py` con `compute_vgp_session(listino, config)` — consumatore di `min_max_normalize` su 3 colonne (ROI, Velocity, Cash_Profit) + applicazione R-05 + R-08 vettoriale; (c) lookup `config_overrides` per soglia ROI runtime override (sbloccato da CHG-031); (d) telemetria evento `veto_roi_applied` con primo orchestrator pipeline; (e) lookup `Referral_Fee` per categoria; (j) F4 Quantità Target + F4.A/B/C; (k) F5; (l) orchestratore di sessione + persistenza Budget_T+1; chiusi: (a/b/f/g/h/i) |
