...
> **Ultimo aggiornamento:** 2026-04-30 — commit `f693abc` (feat(formulas): F4 + F4.A + F5 + velocity_monthly). Tag: `milestone/stack-frozen-v0.9.0` + **`milestone/first-formula-v1.0.0`** + **6 checkpoint** (01..06; 06 su `0812f5d`). Catena CHG odierna: 001→...→**038**. **Tabelle Allegato A coperte: 10/10** ✓ + **346 test PASS** (325 unit/governance + 21 integration). **Indice GitNexus** (1907 / 2203 — stale ~13 commit dopo CHG-034..038 + governance; reindex consigliato).
> **Sessione corrente:** TALOS — **catena scalare delle formule completa F1..F5 + ROI + fee_fba**: `formulas/velocity.py` con F4.A `q_m`, F4 `qty_target`, F5 `qty_final` (Floor lot Samsung MVP=5), `velocity_monthly` (rotazione attesa per VGP). Costanti `DEFAULT_VELOCITY_TARGET_DAYS=15` (L05) e `DEFAULT_LOT_SIZE=5`. **Tutti i building block scalari per l'orchestratore di sessione disponibili**. Cluster `vgp/` (normalize/score/veto) + `tetris/` (allocator/panchina) operativi.
...
| **NEXT** | **Prossimi step possibili** | Catena Leader-driven in corso | (l) **orchestratore di sessione end-to-end** (`session.py` o `orchestrator.py`: precalcolo F1/F2/ROI/F4/F5/velocity_monthly per riga + `compute_vgp_score` + sort + `allocate_tetris` + `build_panchina` + `compounding_t1` + dataclass `SessionResult`); (c) lookup `config_overrides` per soglia ROI runtime override (sbloccato da CHG-031); (d) telemetria evento `veto_roi_applied` + `tetris_break_saturation` (richiede orchestratore); (e) lookup `Referral_Fee` per categoria; chiusi: (a/a'/a''/b/b''/f/g/h/i/j/k) |
