...
> **Ultimo aggiornamento:** 2026-05-01 round 6 — **CHG-032 (B1.1.c): bridge stdlib→structlog su `io_/` + split test telemetria**. HEAD `aeadc98`. Replica pattern CHG-030/031 su 4 file `io_/` (5 emit: `keepa.miss`, `keepa.rate_limit_hit`, 2x `scrape.selector_fail`, `ocr.below_confidence`). Splittato `test_io_extract_telemetry.py` in `test_io_telemetry.py` (7 test LogCapture cluster io_) + `test_io_extract_telemetry.py` ridotto (4 test caplog cluster extract, attesa migrazione B1.1.d). **838 test PASS invariato** (700 + 138). Comportamento applicativo invariato 100%. Detect_changes risk MEDIUM (3 processi cross/intra-community con nodi touched, behavior invariato confermato da 138 integration live e2e). Drift pre-esistente `serp_search.scrape.selector_fail` field names preservato → CHG-B1.4. Catalogo ADR-0021 invariato (17/17). 4 CHG significativi dal checkpoint-16 (CHG-029, 030, 031, 032 — soglia: 5; checkpoint-17 atteso post-B1.1.d). Tag: **8 milestone** + **16 checkpoint** (`checkpoint/2026-05-01-16` su `ceb8c7c`).
