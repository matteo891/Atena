...
> **Ultimo aggiornamento:** 2026-05-01 round 6 — **CHG-031 (B1.1.b): bridge stdlib→structlog su `tetris/` + estrazione fixture conftest**. HEAD `743f7d6`. Replica del pattern CHG-030 su `tetris/{allocator,panchina}.py` (2 emit canonici `tetris.skipped_budget` + `panchina.archived`). Estratta fixture `log_capture` in `tests/conftest.py` (rule-of-three: 4 consumer test post-CHG: `test_logging_config` + `test_vgp_telemetry` + `test_tetris_telemetry` + `test_panchina_telemetry`); rimosse fixture locali. Ruff TC002: `LogCapture` import in `TYPE_CHECKING` block. **838 test PASS invariato** (700 unit/gov/golden + 138 integration). Comportamento applicativo invariato 100%, catalogo ADR-0021 invariato (17/17). 3 CHG significativi dal checkpoint-16 (CHG-029, 030, 031 — soglia: 5). Tag: **8 milestone** + **16 checkpoint** (`checkpoint/2026-05-01-16` su `ceb8c7c`).
