...
> **Ultimo aggiornamento:** 2026-05-01 round 6 — **CHG-030 (B1.1.a): bridge stdlib→structlog su `vgp/`**. HEAD `5c2e92b`. Apertura blocco B1 ("structlog.bind context tracing") ratificato Leader 2026-05-01 round 6 con 5 decisioni pre-flight (1=B/2=a/3=a/4=a/5=a). `vgp/score.py` ora usa `structlog.get_logger` + emit kwargs nativi; `test_vgp_telemetry.py` migrato da pytest `caplog` a `structlog.testing.LogCapture` (pattern `test_logging_config.py`). **838 test PASS** invariato (700 unit/gov/golden + 138 integration con live attivi). Comportamento applicativo invariato 100%, sblocca consumo `merge_contextvars` su 2 emit `vgp.*`. Catalogo eventi canonici ADR-0021 invariato (17/17). 2 CHG significativi dal checkpoint-16 (CHG-029, CHG-030 — soglia: 5). Tag: **8 milestone** + **16 checkpoint** (`checkpoint/2026-05-01-16` su `ceb8c7c`).
