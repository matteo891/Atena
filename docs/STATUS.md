...
> **Ultimo aggiornamento:** 2026-04-30 — commit `82d274f` (feat(ui): pre-save duplicate check). Tag: 3 milestone + 8 checkpoint. Catena CHG odierna: 001→...→**048**. **Tabelle Allegato A coperte: 10/10** ✓ + **430 test PASS** (372 unit/gov/golden + 58 integration). **Indice GitNexus** (1907 / 2203 — stale ~40 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **🎯 UX DUPLICATE-AWARE**: dashboard Streamlit ora chiama `fetch_existing_session_for_listino` post-`run_session`. Se duplicato: warning "Sessione gia' presente: id=X, eseguita...". Bottone "Apri sessione esistente" → load_session_by_id. Pattern check-then-save coerente con idempotency CHG-047 (no IntegrityError visibili al CFO).
