...
> **Ultimo aggiornamento:** 2026-04-30 — commit `98ca62a` (feat(persistence): save_session_result chiude il loop architetturale). Tag: `milestone/stack-frozen-v0.9.0` + `milestone/first-formula-v1.0.0` + `milestone/pipeline-e2e-v1.1.0` + **7 checkpoint**. Catena CHG odierna: 001→...→**042**. **Tabelle Allegato A coperte: 10/10** ✓ + **397 test PASS** (354 unit/governance + 13 golden + **30 integration** — di cui 9 nuovi session_repository). **Indice GitNexus** (1907 / 2203 — stale ~28 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **🎯 LOOP ARCHITETTURALE CHIUSO**: `src/talos/persistence/session_repository.py` con `save_session_result(db_session, *, session_input, result, tenant_id=1) -> int`. Mappa `SessionResult` (intelligenza in-memory di `run_session`) sulle 5 tabelle Postgres Allegato A (`sessions`, `listino_items`, `vgp_results`, `cart_items`, `panchina_items`). Tutto sotto `with_tenant` per future-proof RLS. `listino_hash` deterministico sha256. Pattern Unit-of-Work: caller commit/rollback. Sblocca UI storico ordini, compounding inter-sessione, audit trail completo.
...
| **NEXT** | **Prossimi step possibili** | Loop architetturale chiuso | (t) **integrazione `save_session_result` nella dashboard** (`if st.button("Salva sessione")` + spinner + success message); (u) **`load_session_by_id` + `list_recent_sessions(limit)`** per UI storico (consumer `AnalysisSession`+children); (d) telemetria `session_started`/`tetris_break_saturation`/`veto_roi_applied` con structlog dispatch (ADR-0021); (q) refactor UI multi-page ADR-0016; (r) `io_/extract` Samsung (Playwright + Tesseract + Keepa) — last big block; (v) `UNIQUE(listino_hash)` migration + upsert idempotente; (s) golden Samsung 1000 ASIN; (c) lookup `config_overrides` runtime per soglia ROI; (e) lookup `Referral_Fee` per categoria; chiusi: (a/a'/a''/b/b''/f/g/h/i/j/k/l/m/n/p) |
