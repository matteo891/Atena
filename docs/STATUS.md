...
> **Ultimo aggiornamento:** 2026-05-01 round 5 in corso — **hardening flow descrizione+prezzo CHG-020**: A1 telemetria UI chiusa (CHG-021 `c535065`). HEAD `c535065`. **778 test PASS** (645 unit/gov/golden + 133 integration). **Catalogo eventi canonici ADR-0021: 13/13 voci** (era 10/11; +`session.replayed` drift CHG-058 sanato + `ui.resolve_started` + `ui.resolve_confirmed`). Modalità "macina" round 5 attiva (clausola di sessione, non persiste). Tag invariati: **8 milestone** + **14 checkpoint**.
