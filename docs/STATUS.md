...
> **Ultimo aggiornamento:** 2026-04-30 — commit `316940b` (feat(ui): integra save_session_result in dashboard). Tag: `milestone/stack-frozen-v0.9.0` + `milestone/first-formula-v1.0.0` + `milestone/pipeline-e2e-v1.1.0` + 7 checkpoint. Catena CHG odierna: 001→...→**043**. **Tabelle Allegato A coperte: 10/10** ✓ + **401 test PASS** (369 unit/gov/golden + 32 integration + 1 skipped intenzionale). **Indice GitNexus** (1907 / 2203 — stale ~30 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **🎯 LOOP UI→DB INTEGRATO**: dashboard Streamlit ora chiama `save_session_result` via bottone "Salva sessione su DB" (graceful degrade se `TALOS_DB_URL` assente). Helper `get_session_factory_or_none()` + `try_persist_session(factory, inp, result, tenant_id) -> (success, sid, error)` esposti e testati. Il CFO esegue → vede → salva in 3 click. Pattern idempotente coerente ADR-0016 (niente persistenza automatica).
...
| **NEXT** | **Prossimi step possibili** | Loop UI→DB integrato | (u) **`load_session_by_id` + `list_recent_sessions(limit)`** per UI storico (consumer `AnalysisSession` + children con eager-load relationships); (w) **pagina storico in dashboard** (lista sessioni recenti + click → dettaglio); (d) telemetria `session_started`/`tetris_break_saturation`/`veto_roi_applied` con structlog dispatch (ADR-0021); (q) refactor UI multi-page ADR-0016; (r) `io_/extract` Samsung (Playwright + Tesseract + Keepa) — last big block; (v) `UNIQUE(listino_hash)` migration + upsert idempotente; (s) golden Samsung 1000 ASIN; (c) lookup `config_overrides` runtime per soglia ROI; (e) lookup `Referral_Fee` per categoria; chiusi: (a/a'/a''/b/b''/f/g/h/i/j/k/l/m/n/p/t) |
