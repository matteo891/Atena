...
> **Ultimo aggiornamento:** 2026-04-30 — commit `d2a502a` (feat(persistence,ui): list_recent_sessions + UI storico expander). Tag: 3 milestone + 7 checkpoint. Catena CHG odierna: 001→...→**044**. **Tabelle Allegato A coperte: 10/10** ✓ + **409 test PASS** (369 unit/gov/golden + 40 integration). **Indice GitNexus** (1907 / 2203 — stale ~32 commit; reindex consigliato).
> **Sessione corrente:** TALOS — **🎯 LOOP READ CHIUSO**: `SessionSummary` + `list_recent_sessions(db_session, *, limit, tenant_id)` con subquery `count()` aggregati e tiebreaker `id DESC` su `started_at`. Dashboard Streamlit con expander "Storico Sessioni" via `fetch_recent_sessions_or_empty` (graceful empty). Il CFO ora **esegue → vede → salva → rivede** lo storico in dashboard.
...
| **NEXT** | **Prossimi step possibili** | Loop CRUD core completo | (x) **`load_session_by_id`** (ricostruzione SessionResult dal DB con join multi-tabella + bottone "Ricarica sessione" UI); (d) telemetria `session_started`/`tetris_break_saturation`/`veto_roi_applied` con structlog dispatch (ADR-0021); (q) refactor UI multi-page ADR-0016 (`pages/`, `components/`, `state.py` + caching `@st.cache_data`); (r) `io_/extract` Samsung (Playwright + Tesseract + Keepa) — last big block; (v) `UNIQUE(listino_hash)` migration + upsert idempotente; (s) golden Samsung 1000 ASIN; (c) lookup `config_overrides` runtime per soglia ROI; (e) lookup `Referral_Fee` per categoria; chiusi: (a/a'/a''/b/b''/f/g/h/i/j/k/l/m/n/p/t/u) |
