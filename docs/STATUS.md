...
> **Ultimo aggiornamento:** 2026-04-30 — commit `d0fec07` (chore backfill hash eb04afb + STATUS post CHG-032). Tag: `milestone/stack-frozen-v0.9.0` + **5 checkpoint** (01 / 02 / 03 / 04 / 05 target `cf11e6c`). Catena CHG odierna: 001→...→032. **Tabelle Allegato A coperte: 10/10** ✓ + **256 test PASS** (235 unit/governance + 21 integration). **Indice GitNexus operativo** (1825 / 2113 / 13 / 4).
> **Sessione corrente:** TALOS — config layer pydantic-settings (CHG-029) + 2 consumatori reali (CHG-030 engine, CHG-031 db_bootstrap, centralizzazione completa env var DB) + **catena scalare formule chiusa**: F1 cash_inflow → F2 cash_profit → F3 compounding (CHG-032) + ROI + Veto R-08 + sentinella e2e (CHG-022/025/026/027/028). Chiusura ISS-001 + due errata ADR-0010 (CHG-023/024).
...
| **NEXT** | **Prossimi step possibili** | In attesa | (a) `vgp/normalize.py` versione vettoriale L04b min-max (richiede Numpy/pandas — primo dep applicativo); (b) ~~refactor `engine.py` via `TalosSettings`~~ chiuso CHG-030; (c) lookup `config_overrides` per soglia ROI runtime override (sbloccato da CHG-031); (d) telemetria evento `veto_roi_applied` con primo orchestrator pipeline; (e) lookup `Referral_Fee` per categoria; (f) ~~F3 Compounding T+1~~ chiuso CHG-032; (g) ~~refactor `scripts/db_bootstrap.py` via settings~~ chiuso CHG-031; (h) `milestone/first-formula-v1.0.0` candidato (catena scalare chiusa); (i) estendere sentinella e2e con F3 |
