...
### 2026-05-02 (CHG-2026-05-02-040) — Decisione α'' invertita: fee_fba atomica Keepa preferred

**Decisione originale (CHG-2026-05-01-015 round 4)**: `_LiveKeepaAdapter.query()`
ritorna SEMPRE `fee_fba_eur=None`. Motivazione: il `pickAndPackFee` Keepa
è ~10x più piccolo della formula L11b "Fee FBA totale" Frozen del Leader,
quindi la sostituzione avrebbe rotto Cash_Profit/ROI/VGP.

**Errata Leader 2026-05-02 round 7+**: dopo la calibrazione su ground truth
ScalerBot500K (CHG-2026-05-02-039, file `ordine_scaler500k (22).xlsx`),
si è scoperto che **ScalerBot stesso usa la fee atomica** (~€3) per il
calcolo ROI/cart, non la L11b totale. Su 6 ASIN Samsung reali analizzati:
- ROI con fee atomica: 100% match con ground truth Leader.
- ROI con fee L11b: 5/6 ASIN VETOed (R-08 ROI<8%) → bot inutilizzabile.

**Decisione invertita (default Leader 2026-05-02)**: `_LiveKeepaAdapter`
ora popola `fee_fba_eur` da `product["fbaFees"]["pickAndPackFee"]` Keepa
(cents → EUR via `/100`). Quando Keepa non espone il campo, caller
riceve `KeepaMissError` → fallback `fee_fba_manual` L11b (CHG-022)
SOLO per listini test/dev senza Keepa attiva.

**Pipeline propagation**: `KeepaProduct.fee_fba_eur` →
`ProductData.fee_fba_eur` → `ResolutionCandidate.fee_fba_eur` →
`ResolvedRow.fee_fba_eur` → colonna `fee_fba_eur_keepa` in `listino_raw` →
`orchestrator._enrich_listino` (preferisce Keepa, fallback L11b).

**Test di conformità**: golden CHG-039 sentinel "L11b > implied" PRESERVATO
come anti-regressione (la formula L11b non cambia, cambia solo la
preferenza pipeline). Match exact con ground truth Leader quando Keepa
attiva. `fee_fba_manual` resta come fallback per Samsung MVP no-Keepa.

**Coerenza con ADR-0018**: `cash_inflow_eur` formula F2 invariata; cambia
solo l'input `fee_fba_eur`. ROI/VGP/Cart si allineano automaticamente
con ScalerBot quando Keepa attiva.
