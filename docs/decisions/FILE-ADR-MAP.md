...
| `src/talos/persistence/session_repository.py` | ADR-0015 | ADR-0014, ADR-0019 | `save_session_result(db_session, *, session_input, result, tenant_id=1) -> int` (CHG-042); `SessionSummary` frozen dataclass + `list_recent_sessions(db_session, *, limit=20, tenant_id=1) -> list[SessionSummary]` (CHG-044) — read-side della pipeline DB con tiebreaker `id DESC` su `started_at` |
