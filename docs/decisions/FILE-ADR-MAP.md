...
| `src/talos/io_/scraper.py` | ADR-0017 | ADR-0014, ADR-0019, ADR-0021 | Skeleton `AmazonScraper` con `BrowserPageProtocol` adapter pattern + selector fallback chain CSS→XPath (D2.a default) + UA fisso (D2.b) + delay range fresh context (D2.c) + `parse_eur` italiano/anglo + `SelectorMissError` R-01 + `_PlaywrightBrowserPage` skeleton (`NotImplementedError` + TODO `playwright install chromium`); test mock-only senza Chromium — CHG-2026-05-01-002; +telemetria `scrape.selector_fail` su drift selettori anche con `missing_ok=True` — CHG-2026-05-01-005; + `_PlaywrightBrowserPage` ratificato live: `sync_playwright` + `chromium.launch(headless=True)` + viewport 1920x1080 + `playwright-stealth` (decisione B medium) + cookie consent GDPR `#sp-cc-accept` best-effort (decisione A) + timeout default 60s (decisione B) + context manager + close idempotente — CHG-2026-05-01-012; + `BsrEntry(category, rank)` dataclass + `parse_bsr_text(raw) -> BsrEntry \| None` regex italiano/anglo (rimuove parentetico finale) + `BrowserPageProtocol.query_selector_all_text(selector) -> list[str]` (lista di tutti i match CSS) + `_PlaywrightBrowserPage.query_selector_all_text` live (`page.query_selector_all`) + `ScrapedProduct.bsr_chain: list[BsrEntry]` ordinata dal più specifico al più ampio (sub HTML prima, root in fondo) + `AmazonScraper._resolve_bsr_chain` aggrega `bsr_root` + `bsr_sub` con dedup — CHG-2026-05-01-013 |
...
| `src/talos/io_/fallback_chain.py` | ADR-0017 | ADR-0014, ADR-0019 | `lookup_product(asin, *, keepa, scraper, page, ocr) -> ProductData` orchestra Keepa primario (3 `fetch_*` indipendenti, `KeepaMissError` -> field=None + entry in `notes`) + AmazonScraper fallback opzionale su `buybox_eur`/`title` (invocato solo se entrambi `scraper` e `page` forniti); `KeepaRateLimitExceededError`/`KeepaTransientError` propagati al caller (R-01 fail-fast); `OcrPipeline` parametro placeholder non invocato in CHG-006 (OCR scope = PDF fornitore, non screenshot Amazon); `ProductData` frozen con `sources: dict[str, str]` audit trail field→canale + `notes: list[str]` R-01 trail; nessun nuovo evento canonico (telemetria gia' emessa dai sotto-moduli) — CHG-2026-05-01-006; + `bsr_chain: list[BsrEntry]` propagato da scraper (multi-livello specifico→ampio) + `bsr` valorizzato dal più specifico (`bsr_chain[0].rank`) se Keepa miss; sources tracciano canale per `bsr` e `bsr_chain` separatamente — CHG-2026-05-01-013 |
...
| `src/talos/extract/asin_master_writer.py` | ADR-0017, ADR-0015 | ADR-0014, ADR-0019 | `AsinMasterInput` dataclass + `upsert_asin_master(db, *, data) -> str` con `pg_insert.on_conflict_do_update` (D5.a Postgres-native UPSERT atomico); merge `COALESCE(EXCLUDED.field, asin_master.field)` per i campi nullable (D5.b: input non-null vince, null preserva); `last_seen_at = NOW()` sempre refresh; nessun trigger audit_log (D5.c) — CHG-2026-05-01-005; + `build_asin_master_input(product_data, *, brand, enterprise, samsung_entities, title_fallback, category_node) -> AsinMasterInput` bridge `ProductData` → input writer; title precedence `product_data.title` > `title_fallback` > `ValueError` (R-01 NO SILENT DROPS); `enterprise` param caller prevale (NLP non affidabile per dual-SKU Samsung) — CHG-2026-05-01-008 |
| `src/talos/extract/acquisition.py` | ADR-0017, ADR-0015 | ADR-0014, ADR-0019 | Orchestratore Fase 1 Path B: `acquire_and_persist(asin_list, *, db, keepa, brand, enterprise, scraper, page, extractor, title_fallbacks, category_node) -> list[str]` compone `lookup_products` (CHG-009) → `parse_title` (opzionale) → `build_asin_master_input` (CHG-008) → `upsert_asin_master` (CHG-005). Pattern Unit-of-Work (caller fa commit). Fail-fast su rate-limit/transient/title-None-senza-fallback — CHG-2026-05-01-010 |
