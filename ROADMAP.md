...
| 14 | Persistence skeleton: SQLAlchemy 2.0 deps + plugin `sqlalchemy[mypy]` + `Base = DeclarativeBase` + struttura `migrations/` minima (no modelli, no DDL, no Postgres) | ADR-0015, ADR-0014 | **In corso** (CHG-2026-04-30-007) |
| 15 | Primo modello concreto + Alembic revision iniziale (probabilmente `sessions` o `asin_master`); valutazione Postgres in Docker per integration test | ADR-0015 | Prossimo |
